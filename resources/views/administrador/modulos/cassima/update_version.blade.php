...
function buscar_informacion(iddoc){
   $.post("buscar_info_doc_json",{iddoc:iddoc},function(data){
      ...
      if(data.length != 0){
         $('#gddoc_identificacion').val(data[0].gddoc_identificacion);
         $('#gddoc_descripcion').val(data[0].gdver_descripcion);

         $('#version_actual').val(data[0].gdver_version);
         var new_version = parseInt(data[0].gdver_version)+1;
